...
class JsonApiExtrasFunctionalTest extends JsonApiFunctionalTestBase {
  ...
  public function testResourceTypeFieldAliasing() {
    ...
    $this->assertSame('author', $resourceTypeRepository->getByTypeName('node--article')
      ->getPublicName('uid'));
  }
  ...
  public function testRead() {
    ...
    $this->assertCount(count($this->nodes[5]->get('field_tags')
  ...
  public function testDisabledResourcesShowInAdminList() {
    ...
    $row = $this->assertSession()
      ->elementExists('css', sprintf('#jsonapi-enabled-resources-list table tr:contains("%s")', 'taxonomy_term--' . $vocabulary->id()));
    ...
    $row = $this->assertSession()
      ->elementExists('css', sprintf('#jsonapi-disabled-resources-list table tr:contains("%s")', 'taxonomy_term--' . $vocabulary->id()));
    ...
    $this->drupalGet('/admin/config/services/jsonapi/resource_types');
    $row = $this->assertSession()
      ->elementExists('css', sprintf('#jsonapi-disabled-resources-list table tr:contains("%s")', 'taxonomy_term--' . $vocabulary2->id()));
    ...
    $bundle_info = \Drupal::service('entity_type.bundle.info')
      ->getBundleInfo('entity_test_no_label');
    $this->drupalGet('/admin/config/services/jsonapi/resource_types');
    foreach (array_keys($bundle_info) as $bundle) {
      $this->assertSession()
        ->elementNotContains('css', '#jsonapi-disabled-resources-list', 'entity_test_no_label--' . $bundle);
    }
  }
